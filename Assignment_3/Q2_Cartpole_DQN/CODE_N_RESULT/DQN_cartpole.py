...
class DQN:
    ...
    def train(self, episodes_num=EPISODES_NUM):
        ...
        for episode in range(episodes_num):
            ...
            print(
                "Training: Episode = %d, Length = %d, Global step = %d, Last-100 Means value of steps :%d"
                % (episode, self.episode_length, total_steps, mean_steps))
            # self.summary(value=[tf.Summary.Value(tag="Length of episodes", simple_value=episode_length),])
            # self.summary_writer.add_summary(self.summary, episode)

            # summary1(value=[
            #     tf.Summary.Value(
            #         tag="Length of episodes", simple_value=episode_length),
            # ])
...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        print("Number of steps in testing", episode_length)
        results.append(episode_length)
    print("Means value of steps :", sum(results) / len(results))
