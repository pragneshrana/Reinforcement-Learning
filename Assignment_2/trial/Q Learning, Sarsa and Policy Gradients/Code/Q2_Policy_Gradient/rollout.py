...
class rollout():
    ...
    def include_bias(self,ob):
        return [ob[0], ob[1], 1]

    def chakra_get_action(self,theta, ob, rng=np.random):
        ob_1 = include_bias(ob)
        mean = theta.dot(ob_1)
        return rng.normal(loc=mean, scale=1.)


    def get_mean(self,theta, ob):
        ob_1 = include_bias(ob)
        mean = theta.dot(ob_1)
        return mean

    def compute_logPi(self,action, theta, ob):
        ob_1 = include_bias(ob)
        mean = theta.dot(ob_1)
        diff = action - mean
    ...
    def compute_grad_logPi(self,action, theta, state):
        # I*(a - m)*state
