...
select_GridWorldMap = ['A']
movie_names = ['QL_movieA', 'QL_movieB', 'QL_movieC']
lambda_values = [.7]
...
class GeneratePlots():
    ...
    def plot_avg_steps(self):
        ...
        plt.xlim(0, np.max(episodes_count_b,episodes_count_b,episodes_count_c))
        plt.savefig('combine_plot_of_avg_steps'+str(episodes_count)+'.png', dpi=300)
        # plt.show()
    ...
    def plot_total_return(self):
        ...
        plt.xlim(0,np.max(episodes_count_b,episodes_count_b,episodes_count_c))
        plt.savefig('combine_plot_of_avg_return'+ str(episodes_count)+'.png', dpi=300)
    ...
    def lambda_avg_step(self,GridWorldMap,slambda_val):
        ...
        plt.title('Average Number of steps in Gridworld'+ str(GridWorldMap))
        plt.xlabel('Number of episodes')
    ...
    def lambda_total_return(self,GridWorldMap,slambda_val):
        ...
        plt.legend(['Lambda: 0', 'Lambda: 0.3', 'Lambda: 0.5','Lambda: 0.9', 'Lambda: 0.99', 'Lambda: 1'], loc=0)
        plt.title('Average return per episode in Gridworld '+ str(GridWorldMap))
        plt.xlabel('Number of episodes')
        plt.ylabel('Average Return')
        plt.xlim(0, episodes_count)
...
def GenerateMovie(moviefilename,fps=5):
    '''
    method wil create movies and removes unnecessary files
    '''
    os.system("rm "+str(moviefilename)+".mp4")
    os.system("ffmpeg -r "+str(fps) +"-b 1800 -i _tmp%05d.png "+str(moviefilename)+".mp4")
...
def plot_avg_steps(n_episode,avg_steps,selected_GridWorldMap):
    ...
    plt.title("Average number of steps to goal for problem " + str(selected_GridWorldMap))
    plt.xlabel("Episodes")
    plt.ylabel("Average number of steps")
...
#################################
def SARSA_ALGO(env, selected_world,learning_method, n_episode=1000, iterations=50, gamma=0.9, alpha=0.1, epsilon=0.1,RENDERING=False):
    env.reset()
    Q = defaultdict(lambda: np.zeros(env.n_actions))
    ...
    for selected_world in range(len(select_GridWorldMap)):
        ...
        if(learning_method == 'SARSA_LAMBDA'):    
            for l in range(len(lambda_values)):
                ...
                Q = SARSA_LAMBDA_ALGO(env,select_GridWorldMap[selected_world],learning_method, n_episode=n_episode, iterations=iterations,gamma=0.9, slambda_val=lambda_values[l] ,alpha=0.1, epsilon=0.1,RENDERING=RENDERING)

                if RECORDING:
                    GenerateMovie(movie_names[selected_world], 5)
            ...
            if(learning_method == 'QL'):
                Q = QL_ALGO(env,select_GridWorldMap[selected_world],learning_method, n_episode=n_episode, iterations=iterations,gamma=0.9, alpha=0.1, epsilon=0.1,RENDERING=RENDERING)
            if(learning_method == 'SARSA'):      
                Q = SARSA_ALGO(env,select_GridWorldMap[selected_world],learning_method, n_episode=n_episode, iterations=iterations,gamma=0.9, alpha=0.1, epsilon=0.1,RENDERING=RENDERING)
            ...
            plot_avg_steps(n_episode, avg_number_of_steps,selected_world)
            plot_total_return(n_episode, total_return_per_episode,selected_world)
