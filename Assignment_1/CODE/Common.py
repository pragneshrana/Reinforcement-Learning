...
class simulate():
	...
	def plotiing(self,avg_reward,optimal_Action,epsilon):
		...
		for i in range(len(epsilon)):	
			...
			plt.xlabel('Steps')
			plt.legend(loc='best')
		plt.savefig('reward_epsilon.png')
		plt.show()
		...
		for i in range(len(epsilon)):	
			...
			plt.ylim(0,100)
			plt.legend(loc='best')
		plt.savefig('reward_optimal.png')
		plt.show()

if __name__ == '__main__':
	no_of_bandit_problem=200 # number of bandit problems
	no_of_arms=10 # number of arms in each bandit problem
	no_of_plays=100 # number of times to pull each arm

	reward_result = []
	optimal_action_result = []
	epsilon = [0,0.02,0.5,1]
	temp =[0.01,1,1,10]
	controls = [0.1,2,5]
