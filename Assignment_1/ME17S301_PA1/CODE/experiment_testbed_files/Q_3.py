...
class simulate():
	...
	def ucb1(self,pulled_arm_reward,opt_arm_pull,pulled):
		...
		'''
		for i in range(1,self.no_of_bandit_problem) : 
			ucb_Q=self.estimates[i]+self.controls*(np.sqrt(np.log(pulled)/self.counter_of_pulled_arm[i]))
	...
	def plotiing(self,avg_reward,optimal_Action,epsilon):
		...
		for i in range(len(epsilon)):	
			plt.plot(range(self.no_of_plays),avg_reward[i],label = "$control=$"+str(epsilon[i]))
			plt.rc('text',usetex=True)
			plt.title(r'UCB-1 : Avg Reward Vs Steps')
			plt.ylabel('Avg Reward')
			plt.xlabel('Steps')
			plt.legend(loc='best')
		plt.savefig('reward_ucb.jpg')
		plt.show()
		
		for i in range(len(epsilon)):	
			plt.plot(range(self.no_of_plays),optimal_Action[i],label ="$control=$"+str(epsilon[i]))
			plt.title(r'UCB-1 : $\%$ Optimal Action Vs Steps')
			plt.ylabel(r'$\%$ Optimal Action')
			plt.xlabel('Steps')
			plt.ylim(0,100)
			plt.legend(loc='best')
		plt.savefig('optimal_ucb.jpg')
		plt.show()

if __name__ == '__main__':
	no_of_bandit_problem=2000 # number of bandit problems
	no_of_arms=10 # number of arms in each bandit problem
	no_of_plays=1000 # number of times to pull each arm

	reward_result = []
	optimal_action_result = []
	epsilon = [0,0.02,0.1,1]
	temp =[0.001,0.02,0.5,10]
	controls = [0.1,0.5,1,10]
	...
	#case UCB = 0.1
	epsilon_greedy_simulation = simulate(no_of_bandit_problem=no_of_bandit_problem,no_of_arms = no_of_arms ,no_of_plays = no_of_plays,controls=controls[0],mu=0,sigma=1)
	reward , optimal_action = epsilon_greedy_simulation.bandit_simulation('ucb1')
	reward_result.append(reward)
	optimal_action_result.append(optimal_action)

	#case UCB = 0.5
	epsilon_greedy_simulation = simulate(no_of_bandit_problem=no_of_bandit_problem,no_of_arms = no_of_arms ,no_of_plays = no_of_plays,controls=controls[1],mu=0,sigma=1)
	reward , optimal_action = epsilon_greedy_simulation.bandit_simulation('ucb1')
	reward_result.append(reward)
	optimal_action_result.append(optimal_action)

	#case UCB = 1
	epsilon_greedy_simulation = simulate(no_of_bandit_problem=no_of_bandit_problem,no_of_arms = no_of_arms ,no_of_plays = no_of_plays,controls=controls[2],mu=0,sigma=1)
	reward , optimal_action = epsilon_greedy_simulation.bandit_simulation('ucb1')
	reward_result.append(reward)
	optimal_action_result.append(optimal_action)

	#case UCB = 5
	epsilon_greedy_simulation = simulate(no_of_bandit_problem=no_of_bandit_problem,no_of_arms = no_of_arms ,no_of_plays = no_of_plays,controls=controls[3],mu=0,sigma=1)
	reward , optimal_action = epsilon_greedy_simulation.bandit_simulation('ucb1')
	reward_result.append(reward)
	optimal_action_result.append(optimal_action)
	...
	#plotting
	epsilon_greedy_simulation.plotiing(reward_result,optimal_action_result,controls)
