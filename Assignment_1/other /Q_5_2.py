...
class simulate():
	...
	def plotiing(self,avg_reward,optimal_Action,epsilon):
		...
		for i in range(len(epsilon)):	
			plt.plot(range(self.no_of_plays),avg_reward[i],label = "$temperature=$"+str(epsilon[i]))
			plt.rc('text',usetex=True)
			plt.title(r'Softmax : Avg Reward Vs Steps')
			plt.ylabel('Avg Reward')
			plt.xlabel('Steps')
			plt.legend(loc='best')
		plt.savefig('reward_soft.jpg')
		plt.show()
		
		for i in range(len(epsilon)):	
			plt.plot(range(self.no_of_plays),optimal_Action[i],label ="$temprature=$"+str(epsilon[i]))
			plt.title(r'Softmax : $\%$ Optimal Action Vs Steps')
			plt.ylabel(r'$\%$ Optimal Action')
			plt.xlabel('Steps')
			plt.ylim(0,100)
			plt.legend(loc='best')
		plt.savefig('optimal_soft.jpg')
		plt.show()

if __name__ == '__main__':
	no_of_bandit_problem=2000 # number of bandit problems
	no_of_arms=100 # number of arms in each bandit problem
	no_of_plays=1000 # number of times to pull each arm

	reward_result = []
	optimal_action_result = []
	epsilon = [0,0.02,0.1,1]
	temp =[0.02,0.1,0.5,10]
	controls = [0.1,2,5]
	...
	#plotting
	epsilon_greedy_simulation.plotiing(reward_result,optimal_action_result,temp)
